...
def visualize_data(x, root, edge_index, root_index, y):
    print(f"Features (x): {x}")
    print(f"Root Feature: {root}")
...
def main(obj):
    treePath = "C:\\Users\\priya\\Documents\\ProjectData\\" + obj + "\\data.TD_RvNN.vol_5000.txt"
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez("C:\\Users\\priya\\Documents\\ProjectData\\" + obj + "\\InteractionGraph\\"+id+'.npz', x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y) 

            visualize_data(x_x, rootfeat, tree, rootindex, y)
            return None
